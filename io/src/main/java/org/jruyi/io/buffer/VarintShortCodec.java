...
public final class VarintShortCodec implements IShortCodec {

	public static final IShortCodec INST = new VarintShortCodec();

	private VarintShortCodec() {
	}

	@Override
	public short read(IUnitChain unitChain) {
		return (short) VarintIntCodec.INST.read(unitChain);
	}

	@Override
	public void write(short s, IUnitChain unitChain) {
		VarintIntCodec.INST.write(s & 0xFFFF, unitChain);
	}

	@Override
	public short get(IUnitChain unitChain, int index) {
		return (short) VarintIntCodec.INST.get(unitChain, index);
	}

	@Override
	public void set(short s, IUnitChain unitChain, int index) {
		VarintIntCodec.INST.set(s & 0xFFFF, unitChain, index);
	}
