...
@Component(name = "jruyi.io.tcpclient.connpool", policy = ConfigurationPolicy.REQUIRE, createPid = false)
@Reference(name = ConnPoolEndpoint.CONNPOOL, referenceInterface = ComponentFactory.class, target = "(component.name="
		+ IoConstants.CN_TCPCLIENT_CONNPOOL_FACTORY + ")", strategy = ReferenceStrategy.LOOKUP)
public final class ConnPoolEndpoint extends SessionListener implements
	...
	protected void activate(ComponentContext context, Map<String, ?> properties)
		...
		ComponentFactory factory = (ComponentFactory) context
				.locateService(CONNPOOL);
		ComponentInstance connPool = factory
				.newInstance(normalizeConfiguration(properties));
		ISessionService ss = (ISessionService) connPool.getInstance();
